...
int main() {
    ...
    data = stbi_load("maps/brick_diffuse_map.jpg", &width, &height, &nrChannels, 0);
    if (data) {
    ...
    data = stbi_load("maps/brick_normal_map.jpg", &width, &height, &nrChannels, 0);
    if (data) {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
        glGenerateMipmap(GL_TEXTURE_2D);
    } else {
    ...
    data = stbi_load("maps/brick_height_map.jpg", &width, &height, &nrChannels, 0);
    if (data) {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
        glGenerateMipmap(GL_TEXTURE_2D);
    } else {
    ...
    Model scene("shapes/plane_scene.obj");

    objShader.use();

    objShader.setInt("diffuseMap", 0);
    objShader.setInt("normalMap", 1);
    objShader.setInt("heightMap", 2);

    objShader.setFloat("heightScale", 0.1);
